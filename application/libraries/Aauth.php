...
class Aauth {
	...
	public function login($identifier, $pass, $remember = FALSE, $totp_code = NULL) {
		...
		if ( $query->num_rows() != 0 && $this->verify_password($password, $row->pass) ) {
			...
			$data = array(
				...
				'email' => $row->email,
				'funnel_ready' => $row->key_funnel_ready,
				'funnel_club' => $row->key_funnel_club,
				'is_disable_fbanner' => $row->is_disable_fbanner,
				'loggedin' => TRUE
			);
	...
	public function list_users($group_par = FALSE, $limit = FALSE, $offset = FALSE, $include_banneds = FALSE, $sort = FALSE) {
		...
		if (($_SERVER['REQUEST_METHOD'] == "POST") && isset($_POST['search'])) {
			$this->CI->session->set_userdata('search',$_POST['search']);
		} else if (($_SERVER['REQUEST_METHOD'] == "POST") && !isset($_POST['search'])) {
			$this->CI->session->set_userdata('search','');
		} 
		if (isset($_SESSION['search'])) {
			$this->aauth_db->like('email', $_SESSION['search']);
			$this->aauth_db->or_like('username', $_SESSION['search']);
			$this->aauth_db->or_like('first_name', $_SESSION['search']);
			$this->aauth_db->or_like('last_name', $_SESSION['search']);
		}
    ...
    public function set_key_ready($key) {
		if ($_POST['key_ready'] == '5512-1245-3123-3128') {
			$this->aauth_db->where('id', $_SESSION['id']);
			$this->aauth_db->update($this->config_vars['users'] , ['key_funnel_ready' => 1]);
			return true;
		} else {
			return false;
		}
	...
	public function set_key_club($key) {
		if ($_POST['key_club'] == '1461-0987-8682-7836') {
			$this->aauth_db->where('id', $_SESSION['id']);
			$this->aauth_db->update($this->config_vars['users'] , ['key_funnel_club' => 1]);
			return true;
		} else {
			return false;
		}
    ...
    public function save_fbanner_state(){
		...
		if ($_SESSION['is_disable_fbanner']) {
			$fbanner = 0;
		} else {
			$fbanner = 1;
		}
		...
		$this->aauth_db->where('id', $_SESSION['id']);
		$this->aauth_db->update($this->config_vars['users'] , ['is_disable_fbanner' => $fbanner]);
		$this->CI->session->set_userdata('is_disable_fbanner', $fbanner);
		return true;
	}

    public function get_is_disable_fbanner($user_id = FALSE) {

		$user = $this->get_user($user_id);
		return $user->is_disable_fbanner;
	}
	...
	public function exist_email($email) {
		...
		$query = $this->aauth_db->where('email', $email);
		// $query = $this->aauth_db->get( $this->config_vars['user_variables'] );
		$query = $this->aauth_db->get('aauth_users');
		// var_dump($query->result()[0]->id);die;
		if ($query->num_rows() < 1) {
			return FALSE;
		} else {
	...
	public function update_user_by_email($email,$password,$fname,$lname) {
		$data['first_name'] = $fname;
		$data['last_name'] = $lname;
		// $data['password'] = $password;
		$data['pass'] = $this->hash_password($password, $this->exist_email($email));
		// var_dump($data);die;
		$this->aauth_db->where('email', $email);
		return $this->aauth_db->update($this->config_vars['users'], $data);
